...
class PropertyRepository extends RepositoriesAbstract implements PropertyInterface
{
    ...
    public function getSearchedProperties($filters = [], $params = [])
    {
        $filters = array_merge([
            'type'        => null,
//            'bedroom'     => null,
...
//            'location'    => null,
        ...
        if ($filters['type'] !== null) {
            $this->model = $this->model->whereIn('re_properties.type', $filters['type']);
        }

        if ($filters['building_type'] !== null) {
            $this->model = $this->model->whereIn('re_properties.building_type', $filters['building_type']);
        }


        if ($filters['category_id'] !== null) {
            $this->model = $this->model->whereIn('re_properties.category_id', $filters['category_id']);
        }

        if ($filters['city_id']) {
            $this->model = $this->model->whereIn('re_properties.city_id', $filters['city_id']);
        }
