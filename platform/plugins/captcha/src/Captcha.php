...
class Captcha
{
    ...
    public function display($attributes = [], $options = [])
    {
        ...
        if (!$isMultiple && Arr::get($attributes, 'add-js', true)) {
            $html .= '<script src="' . $this->getJsLink($options) . '" async defer type="application/javascript"></script>';
        }
        ...
        return $html . '<script type="application/javascript">var refreshRecaptcha = function () { grecaptcha.reset(); };</script><div class="g-recaptcha" ' . $this->buildAttributes($attributes) . '></div>';
    }
