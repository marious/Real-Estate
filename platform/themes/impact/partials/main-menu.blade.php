<ul {!! $options !!}}>
    @foreach ($menu_nodes as $key => $row)
        <li>
            <a  href="{{ $row->url }}" target="{{ $row->target }}" title="{{ $row->title }}">
                {{ $row->title }}
    ...
    @endforeach
        ...
        <li class="d-none d-xl-none d-block d-lg-block mt-5 pb-4 border-bottom-0">
            <a href="{{ route('public.account.register') }}" title="إعلن مجانا" class="button border btn-lg btn-block text-center">إعلن مجانا<i class="fas fa-laptop-house ml-2"></i></a>
        </li>
