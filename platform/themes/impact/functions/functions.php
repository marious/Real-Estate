...
add_shortcode('properties-for-sale', 'Properties For sale', 'Properties For sal', function () {
...
add_shortcode('header-map', 'Header Map', 'Header Map', function () {
    return Theme::partial('short-codes.header-map');
});
...
function add_addition_fields_into_form($form, $data)
{
    if (get_class($data) == City::class) {
        $form
            ...
            ->addAfter('is_featured', 'show_in_commercial', 'onOff', [
                'label'         => __('Show In Commercial'),
                'label_attr'    => ['class' => 'control-label'],
                'default_value' => false,
            ])
            ->addAfter('show_in_commercial', 'show_in_residential', 'onOff', [
                'label'         => __('Show In Residential'),
                'label_attr'    => ['class' => 'control-label'],
                'default_value' => false,
            ])
...
function save_addition_city_fields($type, $request, $object)
{
    if (is_plugin_active('location') && in_array($type, [CITY_MODULE_SCREEN_NAME])) {
        ...
        $object->show_in_residential = $request->input('show_in_residential');
        $object->show_in_commercial = $request->input('show_in_commercial');
